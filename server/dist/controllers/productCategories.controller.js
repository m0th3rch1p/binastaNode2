...
const index = async (req, res) => {
    const { response, error } = await (0, queryHelpers_1.execQuery)(TABLE_NAME, "SELECT pc.id, pc.name, pc.slug, pc.image_path, COUNT(p.id) as products_count FROM product_categories pc INNER JOIN products p ON p.category_id = pc.id GROUP BY pc.id", ['id', 'name', 'slug', 'image_path', 'created_at']);
    if (error) {
